...
st.image("UI/data/meme.png", width=300)
...
where the temporal amplification term is multiplied with the borrowed times of the book ($\\text{Count}$).

This formulation amplifies the influence of both interaction frequency and recency, assigning higher ratings to frequently and recently interacted books.
...
st.latex(r"""
\begin{align*}
\text{Hybrid Score} = &\ 0.92 \times \log(1 + \text{CF Score}) \\
                      &+ 0.20 \times \text{Semantic Tags Score} \\
                      &+ 0.80 \times \log(1 + \text{Summary Score}) \\
                      &+ 0.01 \times \log(1 + \text{Themes Score})
\end{align*}
""")
